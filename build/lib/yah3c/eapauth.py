...
class EAPAuth:
    def __init__(self, login_info):
        ...
        self.version_info = '\x06\x07\x4f\x32\x64\x56\x45\x52\x42\x65\x4e\x43\x42\x35\x54\x55\x5a\x75\x49\x67\x78\x2b\x4b\x58\x72\x55\x47\x47\x73\x3d\x20\x20'
    ...
    def send_response_md5(self, packet_id, md5data):
        md5buf = '\x10'+md5(chr(packet_id)+self.login_info['password']+md5data).digest()+self.login_info['username']
        eap_packet = self.ethernet_header + get_EAPOL(EAPOL_EAPPACKET, get_EAP(EAP_RESPONSE, packet_id, EAP_TYPE_MD5, md5buf))
        try:
